...
## Web Deployment
- Export preset: `export_presets.cfg` — preset name "Web", platform "Web", versioned output currently `build/web/dungeon_delver_web_v9_5.html`
- Export command: `/usr/local/bin/godot --headless --export-release "Web"`
- After exporting, copy the versioned HTML to `build/web/index.html`; keep the versioned asset base (`dungeon_delver_web_v9_5`) inside the copied HTML.
...
cp build/web/dungeon_delver_web_v9_5.html build/web/index.html
rm -rf /tmp/dnd-v9-5-web
cp -R build/web /tmp/dnd-v9-5-web
git add -A
git commit -m "Prepare v9.5 ranged and UI update"
git push origin main
git checkout gh-pages
git rm -r --ignore-unmatch .
cp -R /tmp/dnd-v9-5-web/* .
git add -A
git commit -m "Deploy v9.5 web build"
