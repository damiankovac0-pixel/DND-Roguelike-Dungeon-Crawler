...
func _check_game_manager_version() -> void:
	...
	if gm_version != "23.2.0":
		_fail(
			(
				"GameManager.GAME_VERSION expected '23.2.0', got '%s' — V23.2.0 release not set"
				% gm_version
			)
		)
		return
	print("  GameManager.GAME_VERSION = 23.2.0")

	var label: String = gm.get_version_label()
	if not "23.2.0" in label or not "2026-07-11" in label:
		print(
			(
				"  WARNING: GameManager.get_version_label() = '%s' — "
				+ "may not reference 23.2.0 / 2026-07-11" % label
			)
		)
	else:
		print("  GameManager.get_version_label() references 23.2.0 / 2026-07-11")
