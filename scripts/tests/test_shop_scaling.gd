...
func _run() -> void:
	...
	# V23.1.0: stock = 5 + clampi(int(max(1, floor) / 6), 0, 3); min rarity Common<=10, Uncommon 11-17, Rare 18+
	_check_stock(game, 2, 5, 4, ItemDataScript.ItemRarity.COMMON, true)
	_check_stock(game, 10, 6, 14, ItemDataScript.ItemRarity.COMMON, false)
	_check_stock(game, 16, 7, 21, ItemDataScript.ItemRarity.UNCOMMON, false)
